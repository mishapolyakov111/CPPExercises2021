...
void test(std::string name, std::string k) {
    ...
    cv::Mat original1 = cv::imread(full_path);
...
//    cv::imwrite(out_path + "/02_binary_thresholding.jpg", binary);
    ...
    cv::dilate(binary, binary_dilated, cv::getStructuringElement(cv::MORPH_RECT, cv::Size(2,2)));
    ...
    cv::erode(binary_dilated, binary_eroded, cv::getStructuringElement(cv::MORPH_RECT, cv::Size(3, 3)));
    cv::imwrite(out_path + "/05_erode.jpg", binary_eroded);

//    cv::dilate(binary_eroded, binary_dilated, cv::getStructuringElement(cv::MORPH_RECT, cv::Size(4,4)));
//    cv::imwrite(out_path + "/04_erode&dilate.jpg", binary_dilated);
    ...
    cv::findContours(binary, contoursPoints2, cv::RETR_EXTERNAL, cv::CHAIN_APPROX_SIMPLE);
    cv::Mat img1;
    cv::cvtColor(original1, img1, cv::COLOR_BGR2GRAY);
    cv::imwrite(out_path + "/01_grey.jpg", img1);
    cv::adaptiveThreshold(img1, binary, 255, cv::ADAPTIVE_THRESH_GAUSSIAN_C, cv::THRESH_BINARY_INV, 9, 10);
    std::cout << "Contours2: " << contoursPoints2.size() << std::endl;
    cv::Mat imageWithContoursPoints2 = drawContours(img1.rows, img1.cols, contoursPoints2);
